...
export default function Bin2Dec({ darkModeObj }) {
  ...
  const [binaryValue, _setBinaryValue] = useState(
    parseInt(localStorage.getItem(keyBinaryValue)) || 0
  );
  const [decimalValue, _setDecimalValue] = useState(
    parseInt(localStorage.getItem(keyDecimalValue)) || 0
  );
  const [fromIsBinary, setFromIsBinary] = useState(
